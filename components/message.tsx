      ...
      >
        <div className="flex flex-col w-full space-y-3">
          {message.parts?.map((part, i) => {
            switch (part.type) {
              case 'text':
                if (part.text.includes('[Intent') || part.text.includes('[mcp-ui')) {
                  return (
                    <div
                      key={`message-part-${i}`}
                    ...
                    >
                      <div
                        className={cn(
                          'flex items-center gap-3 rounded-full py-2.5 px-4 w-fit',
                          'bg-primary/10 dark:bg-primary/10 ocean:bg-primary/10 text-primary',
                          'border border-primary/20 dark:border-primary/20 ocean:border-primary/20'
                        )}
                      >
                        <div className="text-sm font-semibold tracking-tight">User UI action</div>
                      </div>
                    </div>
                  );
                }
                if (part.text.includes('[iframe]')) {
                  const url = part.text.split('[iframe]')[1].trim();
                  return (
                    <div key={`message-part-${i}`} className="flex flex-row gap-2 items-start w-full">
                      <div className="flex flex-col gap-3 w-full">
                        <iframe src={url} className="w-[800px] h-[600px]" />
                      </div>
                    </div>
                  );
                }
              ...
              case 'reasoning':
                return (
                  <ReasoningMessagePart
                    ...
                    part={part}
                    isReasoning={
                      (message.parts && status === 'streaming' && i === message.parts.length - 1) ??
                      false
                    }
                  />
                );
